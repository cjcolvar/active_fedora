...
namespace :repo do
  ...
  task :delete => :init do
    if ENV["pid"].nil? 
      ...
      puts "Deleted '#{pid}' from #{ActiveFedora.fedora_config[:url]}" if result == 1
    end
  end
  ...
  desc "Load the object located at the provided foxml path or identified by pid. Example: rake repo:load foxml=spec/fixtures/demo_12.foxml.xml"
  task :load => :init do
